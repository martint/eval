...
public class ColumnarVector
{
    ...
    public static void isNotNull(int count, byte[] inputMask, byte[] outputMask, byte[] nulls)
    {
        int i = 0;
        for (; i < BYTE_SPECIES.loopBound(count); i += BYTE_SPECIES.length()) {
            var inputVector = ByteVector.fromArray(BYTE_SPECIES, inputMask, i);
            var nullVector = ByteVector.fromArray(BYTE_SPECIES, nulls, i);

            inputVector.intoArray(outputMask, i, nullVector.eq((byte) 0));
        }

        for (; i < inputMask.length; i++) {
            outputMask[i] = (byte) (inputMask[i] & (nulls[i] == 0 ? 1 : 0));
        }
    }
    ...
    private static void isNull(int count, byte[] inputMask, byte[] outputMask, byte[] nulls)
    {
        ...
        for (; i < BYTE_SPECIES.loopBound(count); i += BYTE_SPECIES.length()) {
            ...
            inputVector.intoArray(outputMask, i, nullVector.eq((byte) 1));
        }

        for (; i < inputMask.length; i++) {
            outputMask[i] = (byte) (inputMask[i] & (nulls[i] == 1 ? 1 : 0));
        }
    }

    public static void greaterOrEqual(int count, byte[] inputMask, byte[] outputMask, long[] values, long value)
    {
